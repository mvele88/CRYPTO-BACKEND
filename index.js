...
// This makes the service more resilient to a single gateway failure.
const IPFS_GATEWAYS = [
  "https://ipfs.io/ipfs/",
...
async function sendBTCFee(amountBTC) {
  try {
    // Note: The Blockonomics API endpoint used here is for creating a new address.
    // To actually send a fee, a different endpoint and more details (like private key) are required.
    // The current implementation is a placeholder for the purpose of logging the event.
    await axios.post(
...
app.get("/api/ipfs", async (req, res) => {
  ...
  // Attempt to fetch content from each gateway in the list.
  ...
  if (response && response.data) {
    ...
    // If all gateways fail, send a 500 status with the last encountered error.
