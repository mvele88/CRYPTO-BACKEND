...
app.get('/api/ipfs/:cid', async (req, res) => {
  ...
  for (const gateway of IPFS_GATEWAYS) {
    try {
      response = await axios.get(`${gateway}${cid}`, { timeout: 5000 }); // Added a 5-second timeout
      break;
    } catch (err) {
  ...
  if (response && response.data) {
    ...
    const errorMessage = lastError ? `Last error from gateway: ${lastError.message}` : 'An unknown error occurred.';
