...
// Cache for IPFS or other endpoints if needed
const cache = {};
...
async function sendBTCFee(amountBTC) {
  try {
    await axios.post(
      ...
      {},
      { headers: { Authorization: `Bearer ${BLOCKONOMICS_API_KEY}`, 'Content-Type': 'application/json' } }
    );
...
// Root route
app.get('/', (req, res) => {
  res.send('<h1>Backend is live</h1><p>Supports BTC and Phantom wallet connections.</p>');
});

// Wallet connect
app.get('/api/wallet-connect', async (req, res) => {
  try {
    const type = req.query.type || 'solana'; // 'btc' or 'solana'
    ...
    if (type === 'btc') {
      ...
      res.json({ connected: true, address: response.data.address });

    } else if (type === 'solana') {
      const connection = new Connection(clusterApiUrl('mainnet-beta'));
      res.json({ connected: true, network: 'mainnet-beta' });
    }
  } catch (err) {
...
app.get('*', (req, res) => res.status(404).json({ message: 'Not Found. Use /api/ipfs or /api/wallet-connect' }));

// Listen locally for dev
